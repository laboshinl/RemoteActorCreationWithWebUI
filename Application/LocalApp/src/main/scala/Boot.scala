...
object Boot extends App {
  ...
  val remoteSystem  = Await.result((supervisor ? Props(classOf[RemoteSystemManager], OSManager)), timeout.duration).asInstanceOf[ActorRef]
  ...
  val web           = Await.result((supervisor ? Props(classOf[WebUIActor], controller, taskManager)),
    timeout.duration).asInstanceOf[ActorRef]
  val heartBleed    = Await.result((supervisor ? Props(classOf[OverAllHeartBleed], List(remoteSystem, routerManager, actorManager))),
    timeout.duration).asInstanceOf[ActorRef]
